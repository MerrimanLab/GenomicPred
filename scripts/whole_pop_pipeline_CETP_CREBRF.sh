...
## bash scripts/whole_pop_pipeline.sh data/data_filtered {POP_NAME} 

## 

# remove duplicate markers and non-snps
# plink --bfile data --exclude remove_dup_markers.txt --snps-only just-acgt --make-bed --out data_filtered
...
software/plink1.9b6.10 --bfile tmp/dataPolySNPS --keep data/${POP}.keep --maf 0.01  --make-bed --out tmp/${POP}_sorteddata
...
do
	...
	software/plink1.9b6.10 --bfile tmp/dataPolySNPS --keep data/${POP}.keep --make-bed --maf 0.01 --out tmp/${POP}_${TRAIT}
