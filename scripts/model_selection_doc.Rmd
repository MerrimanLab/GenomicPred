...
hsq_files <- fs::dir_ls(path = here("results/nph_HEIGHT/GCTA"), glob = "*.hsq.tsv")
...
```{r, warning = FALSE, message = FALSE}
ldak_files <- fs::dir_ls(path = here("results/nph_HEIGHT/LDAK"), glob = "*.reml.h2")
ldak <- purrr::map(ldak_files, read_delim, delim = ' ') %>% set_names(str_remove_all(ldak_files, here("results/nph_HEIGHT/LDAK/")))

ldak_p_files <- fs::dir_ls(path = here("results/nph_HEIGHT/LDAK"), glob = "*.p")
ldak_p <- purrr::map(ldak_p_files, ~ read_delim(.x, delim = " ", col_names = c("stat", "lrt_p"))) %>% 
  set_names(str_remove_all(ldak_p_files, here("results/nph_HEIGHT/LDAK/")))
